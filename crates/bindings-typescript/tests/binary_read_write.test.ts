...
const stripExtraFields = (obj: unknown): unknown => {
  if (Array.isArray(obj)) return obj.map(stripExtraFields);
  if (obj && typeof obj === 'object' && !(obj instanceof Uint8Array)) {
    const result: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(obj)) {
      if (k !== '_0') result[k] = stripExtraFields(v);
    }
    return result;
  }
  return obj;
...
describe('BinaryReader/Writer', () => {
  ...
  test('correctly serializes and then deserializes a complicated web socket message', () => {
    ...
    expect(stripExtraFields(deserializedTransactionUpdate)).toEqual(transactionUpdate);
  });
});
